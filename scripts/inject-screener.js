/**
 * inject-screener.js — adds stock screener section to dashboard
 */
const fs = require('fs');
...
html = html.replace('.n-source { font-size:0.6rem; color:var(--muted); }', 
  '.n-source { font-size:0.6rem; color:var(--muted); }' + screenerCss);

// 2. HTML section (insert before All Listed Companies)
const tableMarker = '📋 All Listed Companies';
const screenerHtml = `
<!-- ====== STOCK SCREENER ====== -->
...
const tableIdx = html.indexOf(tableMarker);
if (tableIdx < 0) { console.log('ERROR: table marker not found'); process.exit(1); }
// Find the start of the section-header div containing this marker
const sectionStart = html.lastIndexOf('<div class="section-header"', tableIdx);
if (sectionStart < 0) { console.log('ERROR: section-header not found'); process.exit(1); }
html = html.slice(0, sectionStart) + screenerHtml + html.slice(sectionStart);

// 3. JS functions (insert before renderCalendar)
const renderCalFn = 'function renderCalendar() {';
...
html = html.replace(renderCalFn, screenerJs + '\n' + renderCalFn);

// 4. Add initScreener call after renderSens in load()
const initCall = "renderSens();\n    render();";
html = html.replace(initCall, "renderSens();\n    initScreener();\n    render();");

fs.writeFileSync(htmlPath, html);
console.log('✅ Screener injected');
