// apply-billion-club.js - Cleanly apply Billion Dollar Club edits to index.html
const fs = require('fs');
...
if (c.includes(styleEnd)) {
  c = c.replace(styleEnd, cssInsert + '\n' + styleEnd);
  changes++;
}

// 2. Add HTML element between heroMarketClose and leftMovers
const heroMC = '<div class="hero-card" id="heroMarketClose"></div>';
const leftMv = '<div class="left-movers" id="leftMovers">';
const newHtml = heroMC + '\n    <div class="hero-card billion-club" id="billionClub"></div>\n    ' + leftMv;
if (c.includes(heroMC + '\n    ' + leftMv)) {
  c = c.replace(heroMC + '\n    ' + leftMv, newHtml);
  changes++;
}

// 3. Add renderBillionClub call to render()
const renderLine = '  renderCurrency(); renderHero(); renderHeroChart(); renderMoversStrip(); renderCalendar();';
const newRenderLine = '  renderCurrency(); renderHero(); renderBillionClub(); renderHeroChart(); renderMoversStrip(); renderCalendar();';
if (c.includes(renderLine)) {
  c = c.replace(renderLine, newRenderLine);
  changes++;
}

// 4. Add renderBillionClub function before renderMoversStrip
...
`;
if (c.includes(moversMarker)) {
  c = c.replace(moversMarker, billionFunc + moversMarker);
  changes++;
}

fs.writeFileSync(f, c);
const kb = (fs.statSync(f).size / 1024).toFixed(1);
console.log('Applied ' + changes + ' changes. File size: ' + kb + 'KB');
